...
synset_num = 10000      # running synset number
batch_size = 64


print('create data start!')
with open('run_data.pkl', 'rb') as f:      # Path
    data = pickle.load(f)
...
LSTM_result = cal_sim(all_synset_des, all_wiki_candidate, model_path='output/training_sts_bilstm-2020-06-08_17-01-37', batch_size=batch_size)      # Path
with open('sim_result/LSTM_{}.pkl'.format(synset_num), 'wb') as f:
    pickle.dump({'model results': LSTM_result}, f)

# The following three models are downloaded to '.cache'
...
xlnet_result = cal_sim(all_synset_des, all_wiki_candidate, model_path='output/training_nli_sts_xlnet', batch_size=batch_size)     # Path
